...
def _update_cinemas_list(path: Union[str, Path] = _json_default_path) -> None:
    ...
    if not exists(path):  # create new json file if doesn't exist
        with open(_path, 'w', encoding='utf-8') as f:
            json.dump(obj=updated_cinemas, fp=f, sort_keys=True, ensure_ascii=False, indent=2)
    else:  # otherwise update existing json (to avoid wiping other cinema chains venues)
        with open(_path, 'r', encoding='utf-8') as f:
            json_output: dict = json.load(f)
            json_output['cinema_city'] = updated_cinemas
        with open(_path, 'w', encoding='utf-8') as f:
            json.dump(obj=json_output, fp=f, sort_keys=True, ensure_ascii=False, indent=2)
        # with tempfile.NamedTemporaryFile(dir='.', delete=False) as temp_file:
        #     # print(type(json_output))
        #     # json.dump(obj=json_output, fp=temp_file, sort_keys=True)
